...
int suinput_mouse(int uinput_fd, int32_t x, int32_t y, int32_t down) {
...
int suinput_open(const char* device_name) {
  ...
  for (i = 0; i < KEY_MAX; i++) {
    if (i != 45 && ioctl(uinput_fd, UI_SET_KEYBIT, i) == -1) // removed key Q
      goto err;
  }
  ...
  user_dev.absmin[ABS_X] = TOUCH_X_MIN;
  user_dev.absmax[ABS_X] = TOUCH_X_MAX;
  user_dev.absfuzz[ABS_X] = 0;
  user_dev.absflat[ABS_X] = 0;

  user_dev.absmin[ABS_Y] = TOUCH_Y_MIN;
  user_dev.absmax[ABS_Y] = TOUCH_Y_MAX;
